...
class parameterized(object):
    ...
    def __call__(self, test_func):
        ...
        if detect_runner() == "pytest":
            import pytest, copy
            argspec = inspect.signature(test_func).parameters
            args = list(argspec.keys())
            ...
            def merge_arg_defaults(p):
                ...
                i = 0
                for arg in args:
                    value_obj = argspec[arg]
                    default_value = value_obj.default
                    try:
                        r = bool(default_value.__name__ == "_empty")
                    except Exception as e:
                        r = False
                    if p.args and i < len(p.args) and r:
                        res.append(p.args[i])
                    elif arg in p.kwargs:
                        res.append(p.kwargs[arg])
                    else:  # 取默认值
