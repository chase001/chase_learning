...
class Print(object):
    ...
    def __init__(self, url='http://qacbz-costpersales.soa.yeshj.com/rest/swagger.json', base_name='Settlement'):
    ...
    def write_file(self, data, file_name='test.txt', mode='w'):
        file_path=None
        if isinstance(data, dict):
            data = json.dumps(data)

        if sys.platform == "win32" or sys.platform == "win64":
            file_path = os.getcwd() + '\\temp_file\\'
        else:
            file_path = os.getcwd() + '/temp_file/'
    ...
    class RestfulApi(object):
        def __init__(self, uri=None, method=None, api_info=None, base_name=None, print_instance=None):
            ...
            self.uri = uri
            self.api = api_info
            self.class_name = self.get_class_name() if self.uri else None
        ...
        def analyze_api_body(self):
            ...
            self.c.write("BaseObj.__init__(**kwargs)\n")
...
if __name__ == '__main__':
    ...
    p = Print(url='http://144.34.200.237:8080/v2/api-docs', base_name="Order")
