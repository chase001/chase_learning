...
class AssertObj(object):
    ...
    def reload(self, pw):
        ...
        for attri in self.__dict__.keys():
            if str(attri).startswith("_") or attri in del_attr:
                pass
            else:
...
class CompareModel(object):
    ...
    def init_info(self, **kwargs):
        ...
        :return:
        """
        # from .RedeemInfo import RedeemInfo
        # self.info_obj = RedeemInfo(activity_obj=activity_obj,
        #                            generate_obj=generate_obj,
        #                            receive_obj=receive_obj,
        #                            )
        pass
        return self
    ...
    def init_pw(self):
        ...
        :return:
        """
        # from .RedeemPeeweeUtil import RedeemPeeweeUtil
        # self.pw = RedeemPeeweeUtil(act_id=self.info_obj.act_id)
        pass
    ...
    def add_table(self, table_name_key, **kwargs):
        """
        根据表名请求计算的表结构和值,同时请求数据库中的值,用于后面做对比
