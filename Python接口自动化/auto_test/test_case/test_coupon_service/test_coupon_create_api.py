...
@parameterized([
    param(req_data=dict(amount=100.99),
          msg="验证amount=100.99时创建优惠券成功"),
    param(req_data=dict(amount=100),
          msg="xxxxx"),
    param(req_data=dict(amount=1),
          msg="xxxxx"),
    param(req_data=dict(amount=999.9),
          msg="xxxxx"),
])  # 第三方库参数化风格
@case_model()
...
@parameterized([
    ...
    param(input=dict(startTime=now(days=1, format=f),
                     enableTime=now(days=31, format=f)),
          msg="跨月"),
    param(input=dict(startTime=now(days=1, format=f),
                     enableTime=now(days=365, format=f)),
          msg="跨年"),
    param(input=dict(startTime=now(days=-5, format=f),
                     enableTime=now(days=-1, format=f)),
          msg="报错1"),
    param(input=dict(startTime=now(days=6, format=f),
                     enableTime=now(days=5, format=f)),
          msg="报错2", code=-1),
])  # 第三方库参数化风格
@case_model()
